...
function render(fitToData) {
  ...
  for (const [key, rows] of areaMap.entries()) {
    ...
    if (rows.length > 1) {
      bubbleCandidates.push({lat:first.lat, lon:first.lon, rows});
    } else {
  ...
  const bubbleCellDeg = zoom <= 4 ? 8 : zoom <= 5 ? 4 : zoom <= 6 ? 2.25 : zoom <= 7 ? 1.1 : zoom <= 8 ? 0.55 : 0;
  let groupedBubbleCandidates;
  if (bubbleCellDeg > 0) {
    const grouped = new Map();
    for (const item of bubbleCandidates) {
      const gy = Math.round(item.lat / bubbleCellDeg);
      const gx = Math.round(item.lon / bubbleCellDeg);
      const gk = `${gy}|${gx}`;
      if (!grouped.has(gk)) grouped.set(gk, []);
      grouped.get(gk).push(item);
    }
    groupedBubbleCandidates = [...grouped.values()].map(group => {
      const allRows = group.flatMap(g => g.rows);
      const lat = group.reduce((s,g)=>s+g.lat,0) / group.length;
      const lon = group.reduce((s,g)=>s+g.lon,0) / group.length;
      return {lat, lon, rows: allRows};
    });
  } else {
    groupedBubbleCandidates = bubbleCandidates;
  }

  for (const bubble of groupedBubbleCandidates) {
    const uniquePeople = new Set(bubble.rows.map(r => r.pid));
    const size = uniquePeople.size >= 100 ? 38 : uniquePeople.size >= 25 ? 34 : 30;
    const icon = L.divIcon({className:'', html:`<div class="area-bubble" style="width:${size}px;height:${size}px;">${uniquePeople.size}</div>`, iconSize:[size,size], iconAnchor:[size/2,size/2]});
    const m = L.marker([bubble.lat, bubble.lon], {icon}).addTo(bubbleLayer); m.bindPopup(popupHtmlForArea(bubble.rows));
  }
...
let renderQueued = false, pendingFitToData = false, renderTimer = null, rerenderRequested = false;
...
document.getElementById('modeSelect').addEventListener('change', e=>{ state.mode = e.target.value; state.selectedBranches = new Set(); state.initializedDates = false; state.branchUiSig = null; showLoading('Updating map scope…', 'Switching scope can be heavy for the full GEDCOM.'); scheduleRender(true, 10); });
['showDirect','showSiblings','showSpouses'].forEach(id=>document.getElementById(id).addEventListener('input', ()=>scheduleRender(false)));
['genDepth','dateMin','dateMax'].forEach(id=>document.getElementById(id).addEventListener('input', ()=>scheduleRender(false, 75)));
document.querySelectorAll('input[name="side"]').forEach(el=>el.addEventListener('change', ()=>{ state.branchUiSig = null; showLoading('Updating family side…', 'Redrawing the visible family lines and points.'); scheduleRender(false, 10); }));
